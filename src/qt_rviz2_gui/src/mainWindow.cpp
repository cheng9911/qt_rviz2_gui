...
MainWindow::MainWindow(QApplication *app, rviz_common::ros_integration::RosNodeAbstractionIface::WeakPtr rviz_ros_node, QWidget *parent)
    ...
    // connect(updateBtn, &QPushButton::clicked, this, &MainWindow::updateFrame);

    // layout->addWidget(frameLabel);
    // layout->addWidget(frameLineEdit_);
    // layout->addWidget(updateBtn);

    // renderPanel_ = new rviz_common::RenderPanel(this);
    // layout->addWidget(renderPanel_);

    // setupJoystickControls(layout);
...
void MainWindow::onInitializeRViz()
{
    ...
    // 发布器
    cmdVelPublisher_ = node->get_raw_node()->create_publisher<geometry_msgs::msg::Twist>(
        "/cmd_vel", 10);
}
...
void MainWindow::updateFrame() {
     QString frame_id = frameLineEdit_->text();
    // // 添加线程安全检测
    // if (manager_->getThreadedQueue()->isEmpty()) { 
    //     manager_->stopUpdate();
